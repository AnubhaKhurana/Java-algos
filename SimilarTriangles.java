...
public class SimilarTriangles {
	...
	int countDistinctTriangles(ArrayList<Sides> arr) {
		...
		int totalDistinctTriangles = 0;

		List<Integer> listObj = new ArrayList<Integer>();

		for (int i = 0; i < arr.size(); i++) {
			Sides triangleObj = arr.get(i);

			int[] sidesToArray = { triangleObj.a, triangleObj.b, triangleObj.c };
			Arrays.sort(sidesToArray);

			int aTriangle = (100 * sidesToArray[0]) + (10 * sidesToArray[1])
					+ sidesToArray[2];

			if (!listObj.contains(aTriangle))
				listObj.add(aTriangle);
		}
		return listObj.size();
	}
