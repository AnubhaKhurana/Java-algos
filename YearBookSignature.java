 ...
 * */
public class YearBookSignature {
	// Add any helper functions you may need here
	  

	  int[] findSignatureCounts(int[] arr) {
	    // Write your code here
		  int[] noOfSignatures = new int[arr.length];
		  
		  for(int i=0; i<arr.length; i++){
			  if(noOfSignatures[i] != 0) continue;
			  int j = i;			  
			  ...
			  do{
				  setObj.add(j);
				  j = arr[j]-1;
			  } while(j != i && !setObj.contains(j ));

		  	if(!setObj.contains(j))
		  		setObj.add(j);
			
			noOfSignatures[i] = setObj.size();
			
			//The number of signatures for each value in Set would be setobj.size()
			setObj.forEach((Integer value) -> noOfSignatures[value] = setObj.size());  
		  }
		  return noOfSignatures;
	  ...
	  public void run() {
	    ...
	    int[] arr_2 = {5, 3, 2, 1, 4};
	    int[] expected_2 = {3, 2, 2, 3, 3};
	    int[] output_2 = findSignatureCounts(arr_2);
	    check(expected_2, output_2);
	  
	    // Add your own test cases here
	    
	    int[] arr_3 = {4, 3, 5, 1, 6, 2};
	    int[] expected_3 = {2, 4, 4, 2, 4, 4};
