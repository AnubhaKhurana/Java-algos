 ...
 * 
 * */
public class BalancedSplitSubSet {
	boolean balancedSplitExists(int[] arr) {
	    // Write your code here
	    int totalValueOfWholeArray = 0;
	    
	    for(int i=0; i<arr.length; i++){
	      totalValueOfWholeArray += arr[i];
	    }
	    
	    if(totalValueOfWholeArray % 2 != 0)
	      return false;
	    Arrays.sort(arr);
	    
	    int totalOfBalancedSubset = totalValueOfWholeArray/2;
	    
	    int internalTotal = 0;
	    for(int i=0; i<arr.length; i++){
	       internalTotal += arr[i];
	       if(internalTotal < totalOfBalancedSubset){
	         continue;
	       } else if(internalTotal == totalOfBalancedSubset){
